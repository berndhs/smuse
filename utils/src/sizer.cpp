...
Sizer::report(QTextStream &output)
{
  ...
  for (auto m=m_mimeRecs.begin(); m!= m_mimeRecs.end(); ++m) {
    ...
    double frag = double(m->totalFrag);
    totalFrag += frag;
    output <<"\t" << frag << " frag bytes "
               << double(frag)/1024.0 << " KiB "
               << double(frag)/(1024.0*1024.0) << " MiB "
               << double(frag)/(1024.0*1024.0*1024.0) << " GiB"<< endl;

  }
  output << "total size " << totalSz << " frag " << totalFrag;
  double frag = totalFrag;
  output <<"\t" << frag << " frag bytes "
             << double(frag)/1024.0 << " KiB "
             << double(frag)/(1024.0*1024.0) << " MiB "
             << double(frag)/(1024.0*1024.0*1024.0) << " GiB"<< endl;
  output << " % is " << 100.0 * totalFrag / totalSz << endl;
